...
Spawn `prepare-delivery:prepare-delivery-agent` with the arguments. It runs the whole pipeline and ends its reply with a `=== PREPARE_DELIVERY_RESULT ===` block. If Task is not available, load the `prepare-delivery` skill (or read `skills/prepare-delivery/SKILL.md` from this plugin) and run it inline.
